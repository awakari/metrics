...
func (c controller) SetMostReadLimits(ctx context.Context, req *SetMostReadLimitsRequest) (resp *SetMostReadLimitsResponse, err error) {
    ...
    if c.svcMetrics != nil {
        ...
        rateSum, err = c.svcMetrics.GetRateAverage(ctx, "awk_reader_read_count", "service")
        if err == nil {
            rateBySrc, err = c.svcMetrics.GetRelativeRateByLabel(ctx, rateSum, "awk_reader_sources_read_count", "source")
        }
    }
    if err == nil && len(rateBySrc) > 0 {
        for srcUrl, rateRel := range rateBySrc {
            if rateRel.Day > 0 {
                ...
                if groupId == "" {
                    fmt.Printf("SetMostReadLimits: source %s is not known, attempting to follow it using ActivityPub\n", srcUrl)
                    srcUrl, err = c.svcAp.Create(ctx, srcUrl, c.groupIdDefault, srcUrl)
                    if err != nil {
                        fmt.Printf("SetMostReadLimits: source %s: failed to follow using ActivityPub: %s\n", srcUrl, err)
                        continue
                    }
